...
export const factoryQueryKeys = {
  ...
  config: (contractAddress: string | undefined, args?: Record<string, unknown>) => ([{ ...factoryQueryKeys.address(contractAddress)[0],
    method: "config",
    args
  }] as const)
};
...
export function useFactoryConfigQuery<TData = ConfigResponse>({
  ...
  return useQuery<ConfigResponse, Error, TData>(factoryQueryKeys.config(client?.contractAddress), () => client ? client.config() : Promise.reject(new Error("Invalid client")), { ...options,

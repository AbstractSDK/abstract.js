...
function getTsConfig(baseUrl: string) {
  return {
    compilerOptions: {
      ...
      noEmit: true,
      // paths: {
      //   '@abstract-money/cli': [path.relative(baseUrl, 'packages/cli/src')],
      //   '@abstract-money/cli/*': [path.relative(baseUrl, 'packages/cli/src/*')],
      //   '@abstract-money/connectors': [
      //     path.relative(baseUrl, 'packages/connectors/src'),
      //   ],
      //   '@abstract-money/connectors/*': [
      //     path.relative(baseUrl, 'packages/connectors/src/*'),
      //   ],
      //   '@abstract-money/core': [path.relative(baseUrl, 'packages/core/src')],
      //   '@abstract-money/core/*': [path.relative(baseUrl, 'packages/core/src/*')],
      //   'abstract-money': [path.relative(baseUrl, 'packages/react/src')],
      //   'abstract-money/*': [path.relative(baseUrl, 'packages/react/src/*')],
      // },

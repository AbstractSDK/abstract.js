...
import { type AssetInfoBaseForAddr } from '../account/proxy/Proxy.types'
...
export class AbstractAccountQueryClient implements IAbstractAccountQueryClient {
  ...
  constructor({ abstract, accountId, managerAddress, proxyAddress }: IAbstractAccountQueryClient) {
  ...
  public async getNamespace(): Promise<string | null> {
    ...
    const namespace = await registryQueryClient
      .namespaces({ accounts: [this.accountId] })
  ...
  public async getBaseAsset(): Promise<AssetInfoBaseForAddr> {
    return await this.proxyQueryClient.baseAsset().then(({ base_asset }) => base_asset)
  }
   ...
   * @param queryMsg
   */
  public async queryModule({ moduleId, moduleType, queryMsg }: QueryModule): Promise<JsonObject> {
    ...
    return await this.abstract.client.queryContractSmart(moduleAddress, moduleQueryMsg)
  }
   ...
   */
  public connectAbstractClient(abstract: AbstractClient): AbstractAccountClient {
  ...
  public connectSigningClient(
    ...
    const signingAbstract = this.abstract.connectSigningClient(signingClient, sender)
...
export class AbstractAccountClient extends AbstractAccountQueryClient {
  ...
  get managerClient(): ManagerClient {
    return new ManagerClient(this.abstract.client, this.sender, this.managerAddress)
  }
  ...
  public managerMsgComposer(sender?: string): ManagerMessageComposer {
    return new ManagerMessageComposer(sender || this.sender, this.managerAddress)
  }
   ...
   *   ContractMsg.
   */
  public buildExecuteOnModuleMsg<T extends ContractMsg>(moduleId: string, execMsg: T): ContractMsg {
  ...
  private wrapModuleExecMsg(execMsg: ContractMsg, moduleType?: ModuleType) {
    return match(moduleType)
      ...
      .with('adapter', () => AdapterExecuteMsgFactory.executeAdapter({ request: execMsg }))
      .otherwise(() => execMsg)
  }
   ...
   */
  public static fromQueryClient(queryClient: AbstractAccountQueryClient, abstract: AbstractClient) {

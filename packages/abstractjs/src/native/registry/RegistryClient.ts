import { type RegistryInterface, type RegistryReadOnlyInterface } from '../registry'
import {
...
import { type CosmWasmClient, type SigningCosmWasmClient } from '@cosmjs/cosmwasm-stargate'
...
export class RegistryQueryClient extends ARegistryQueryClient implements IRegistryQueryClient {
   ...
   * @param moduleInfo
   */
  protected _getCachedModule(moduleInfo: VcModuleInfo): AbstractModule | undefined {
    ...
    const version = match(moduleInfo.version)
      ...
      .with('latest', () => 'latest')
    return this._moduleCache.get([moduleInfo.namespace, moduleInfo.version, version].join(':'))
  }
    ...
    while (hasNextPage) {
      moduleResponses.push(
        ...(await this.moduleList({
          ...
          startAfter = modules[modules.length - 1]?.module?.info
          return modules
        }))
      )
    }
